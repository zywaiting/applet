package com.zy.applet.wechatbot.api;

import com.google.gson.JsonObject;
import com.google.gson.reflect.TypeToken;
import com.zy.applet.utils.OssUploadFileUtils;
import com.zy.applet.wechatbot.WeChatBot;
import com.zy.applet.wechatbot.api.client.BotClient;
import com.zy.applet.wechatbot.api.constant.Constant;
import com.zy.applet.wechatbot.api.constant.StateCode;
import com.zy.applet.wechatbot.api.enums.AccountType;
import com.zy.applet.wechatbot.api.enums.ApiURL;
import com.zy.applet.wechatbot.api.enums.RetCode;
import com.zy.applet.wechatbot.api.model.*;
import com.zy.applet.wechatbot.api.request.BaseRequest;
import com.zy.applet.wechatbot.api.request.FileRequest;
import com.zy.applet.wechatbot.api.request.JsonRequest;
import com.zy.applet.wechatbot.api.request.StringRequest;
import com.zy.applet.wechatbot.api.response.*;
import com.zy.applet.wechatbot.exception.WeChatException;
import com.zy.applet.wechatbot.utils.*;
...
import static com.zy.applet.wechatbot.api.constant.Constant.*;
...
//                }
    ...
    private String downloadImg(String msgId) {
        return this.downloadFile(
                new DownLoad(ApiURL.IMAGE, bot.session().getUrl(), msgId, bot.session().getSKey())
                        .msgId(msgId).saveByDay().suffix("jpg")
        );
    }
    ...
    private String downloadVideo(String msgId) {
        return this.downloadFile(
                new DownLoad(ApiURL.VIDEO, bot.session().getUrl(), msgId, bot.session().getSKey())
                        .msgId(msgId).saveByDay().suffix("mp4")
        );
    }
    ...
    private String downloadVoice(String msgId) {
        return this.downloadFile(
                new DownLoad(ApiURL.VOICE, bot.session().getUrl(), msgId, bot.session().getSKey())
                        .msgId(msgId).saveByDay().suffix("mp3")
        );
    }
    ...
    private String downloadFile(DownLoad downLoad) {
        ...
        try {
            audioUrl = OssUploadFileUtils.OssUploadFileInputStreamtest("wq-zy",
                    "tencentBot/" + downLoad.getSuffix() + "/" + UUID.randomUUID().toString().replaceAll("-", "") + "." + downLoad.getSuffix(),
                    inputStream);
        } catch (FileNotFoundException e) {
            e.printStackTrace();
        }
